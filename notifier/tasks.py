...
def do_forums_digests_flagged(input_file, is_dry_run):
    """
    ...
    def get_input_command():
        ...
        """
        command = settings.HEROKU_COMMAND_FLAGGED.split(' ')
    ...
    def send(batch, is_dry_run):
        if is_dry_run:
            for message in batch:
                email_address = message.get('recipient', {}).get('email')
                course_id = message.get('course_id')
                print("{course_id} {email_address}".format(
                    course_id=course_id,
    ...
    def send_in_batches(messages, is_dry_run):
        """
    ...
    send_in_batches(messages, is_dry_run)
